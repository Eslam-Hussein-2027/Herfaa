...
import { LayoutDashboard, BadgeCheck, Users, LayoutGrid, Star, HelpCircle, LogOut } from 'lucide-react'
import Logo from '@/components/Logo'
import { useAuth } from '@/context/AuthContext'

const LINKS = [
  { to: '/admin', label: 'لوحة التحكم', Icon: LayoutDashboard, end: true },
  { to: '/admin/providers', label: 'اعتماد الحِرفيّين', Icon: BadgeCheck },
      ...
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-e border-ink-200 bg-white md:flex">
        ...
        <div className="border-t border-ink-100 p-3">
          <div className="mb-1 flex items-center gap-2 px-1 py-2">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-teal-100 text-sm font-bold text-teal-700">
              {user?.name?.charAt(0)}
            </span>
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-ink-800">{user?.name}</p>
              <p className="text-xs text-ink-400">مدير</p>
            </div>
          </div>
          <button
            onClick={logout}
